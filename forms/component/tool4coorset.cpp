...
void Tool4CoorSet::on_btnRecord_clicked()
{
    ...
    if (!FormUtils::MessageBoxYesNo(QStringLiteral("记录坐标系数据（G054 - %1），是否运行？").arg(Tool4SwitchCoor::GetCoorName(csNow)))) {
        return;
    }
...
{
    if (!FormUtils::MessageBoxYesNo(QStringLiteral("恢复数据到坐标系，是否运行？"))) {
        return;
    }
