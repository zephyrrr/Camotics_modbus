    ...
    ui(new Ui::WeiZhiJiYiForm), inButtons{ NULL }, inButtons2{ NULL }, m_currentRow(-1), m_currentColumn(-1)
{
    ...
    connect(tabBar, &QTabBar::currentChanged, [this](int index) {
        ...
        for (int j = 0; j < POS_SET_COUNT_PERPAGE; ++j) {
            this->inButtons[j]->setText(QStringLiteral("记忆%1%2").arg(tabIndex == 0 ? "" : QString::number(tabIndex)).arg(j));
            this->inButtons2[j]->setText(QStringLiteral("移动%1%2").arg(tabIndex == 0 ? "" : QString::number(tabIndex)).arg(j));
        }
        });
    ...
    for (int row = 1; row <= POS_SET_COUNT_PERPAGE + 1; ++row) {
        ...
        if (row != POS_SET_COUNT_PERPAGE + 1) {
            QPushButton* button = new QPushButton(QStringLiteral("记忆%1").arg(row), table);
            ...
            QPushButton* button2 = new QPushButton(QStringLiteral("移动%1").arg(row), table);
    ...
    if (filePath == NULL) {
        ...
        for (int row = 1; row <= POS_SET_COUNT_PERPAGE + 1; ++row)
        {
            ...
            else
            {
                ...
                {
                    if (i == 0) {
                        inEdits[row - 1][i]->setText(QStringLiteral("机械"));
                    }
                    else {
...
void WeiZhiJiYiForm::SaveData(QString filePath)
{
    if (filePath == NULL) {
        ...
        for (int row = 0; row <= POS_MEMORY_LEN; ++row)
        {
            ...
            if (row == 0) {
                jsonArrayRow.append(QJsonValue(QStringLiteral("坐标系")));
