                    ...
                    if(metaProperty.isValid() && metaProperty.isEnumType()) {
                        ...
                        if(numKeys > 0) {
                            ...
                            for(int j = 0; j < numKeys; ++j) {
                                ...
                                if (propertyModel->enumHeaders.contains(key))
                                  editor->addItem(propertyModel->enumHeaders[key]);
                                else
                                  editor->addItem(QString(key));
                            }
                            QByteArray currentKey = QByteArray(metaEnum.valueToKey(value.toInt()));
                            if (propertyModel->enumHeaders.contains(currentKey))
                              editor->setCurrentText(propertyModel->enumHeaders[currentKey]);
                            else
                ...
                if(comboBoxEditor) {
                    ...
                    if(propertyModel) {
                        ...
                        if(metaProperty.isValid() && metaProperty.isEnumType()) {
                            ...
                            bool ok;
                            int selectedValue;
                            if (propertyModel->enumHeadersReverse.contains(selectedKey))
                              selectedValue = metaEnum.keyToValue(propertyModel->enumHeadersReverse[selectedKey], &ok);
                            else
                              selectedValue = metaEnum.keyToValue(selectedKey.toLatin1().constData(), &ok);
            ...
            if(value.type() == QVariant::Bool) {
                ...
                if(propertyModel) {
                    ...
                    if(metaProperty.isValid() && metaProperty.isEnumType()) {
                        ...
                        if (propertyModel->enumHeaders.contains(currentKey))
                          itemOption.text = propertyModel->enumHeaders[currentKey];
                        else
