...
{
    if (simpleMode) {
		throw std::runtime_error("简单模式下不支持任务线程模式");
    }
    ...
    if (checkContinue) {
        if (!m_taskToContinue) {
            if (!m_exceptionThrown) {
                ...
                throw GCodeInterruptException(EUtils::QString2StdString(QStringLiteral("用户取消")));
                //throw cb::Exception(EUtils::QString2StdString(QStringLiteral("用户取消")), this->getLocation().getStart());
            }
            else {
                return;
            }
